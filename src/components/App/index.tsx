...
export default class App extends Component<Props, State> {
  ...
  async onFileChange(event: Event): Promise<void> {
    ...
    const side:ImageSide = (fileInput.classList.contains('left')) ?
      ImageSide.left : ImageSide.right;
  ...
  async onFileDrop(event: FileDropEvent) {
    const { file } = event;
    const side:ImageSide = ((event.target as HTMLElement).classList.contains('left')) ?
      ImageSide.left : ImageSide.right;
    if (!file) return;
    await this.updateFile(file, side);
  }
  ...
  async updateFile(file: File, side:ImageSide) {
    const newState = this.state; // this feels dirty
  ...
  render({ }: Props, { leftLoaded, rightLoaded, error, leftImage, rightImage }: State) {
    ...
    const leftImageAttrs = leftImage ?
      { height: leftImage.height, width: leftImage.width } : { width: 300, height: 200 };
    const rightImageAttrs = rightImage ?
      { height: rightImage.height, width: rightImage.width } : { width: 300, height: 200 };
